...
nc_lw_w <- nb2listw(ncCR85, style="W") # row-standardized for no intercept
...
all.equal(c(m1c.imps[[1]]$indirect["ft.NWBIR74"],
...
lc <- summary(glht(m1c, linfct = c("east + lag.east = 0")))
all.equal(c(m1c.imps[[1]]$total["east"],
...
all.equal(c(m1d.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc <- summary(glht(m1d, linfct = c("east + lag.east = 0")))
all.equal(c(m1d.imps[[1]]$total["east"],
...
all.equal(c(m1e.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc <- summary(glht(m1e, linfct = c("east + lag.east = 0")))
all.equal(c(m1e.imps[[1]]$total["east"],
...
# all.equal(c(m1f.imps[[1]]$indirect["ft.NWBIR74"],
...
# 
# # Total impact is linear combination
# lc <- summary(glht(m1f, linfct = c("east + lag.east = 0")))
# all.equal(c(m1f.imps[[1]]$total["east"],
...
# Impact equals coef
all.equal(spatialreg::impacts(m2b)[[1]]$indirect, m2b$coefficients["lag.ft.NWBIR74"], 
          tolerance = 1e-5, check.attributes = FALSE)
...
all.equal(c(m2c.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc <- summary(glht(m2c, linfct = c("east + lag.east = 0")))
all.equal(c(m2c.imps[[1]]$total["east"],
...
all.equal(c(m2d.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc1 <- summary(glht(m2d, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
lc2 <- summary(glht(m2d, linfct = c("east + lag.east = 0")))
all.equal(c(m2d.imps[[1]]$total["ft.NWBIR74"],
...
all.equal(c(m2e.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc1 <- summary(glht(m2e, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
lc2 <- summary(glht(m2e, linfct = c("east + lag.east = 0")))
all.equal(c(m2e.imps[[1]]$total["ft.NWBIR74"],
...
all.equal(c(m2f.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc1 <- summary(glht(m2f, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
lc2 <- summary(glht(m2f, linfct = c("east + lag.east = 0")))
all.equal(c(m2f.imps[[1]]$total["ft.NWBIR74"],
...
all.equal(m3b.imps[[1]]$indirect, m3b$coefficients["lag.ft.NWBIR74"], 
          tolerance = 1e-5, check.attributes = FALSE)

# Total impact is linear combination
lc <- summary(glht(m3b, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
...
all.equal(c(m3c.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc1 <- summary(glht(m3c, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
lc2 <- summary(glht(m3c, linfct = c("east + lag.east = 0")))
all.equal(c(m3c.imps[[1]]$total["ft.NWBIR74"],
...
all.equal(c(m3e.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc1 <- summary(glht(m3e, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
lc2 <- summary(glht(m3e, linfct = c("east + lag.east = 0")))
all.equal(c(m3e.imps[[1]]$total["ft.NWBIR74"],
...
all.equal(c(m3e.imps[[1]]$total["AREA"]), 
...
all.equal(c(m3f.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc1 <- summary(glht(m3f, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
lc2 <- summary(glht(m3f, linfct = c("east + lag.east = 0")))
all.equal(c(m3f.imps[[1]]$total["ft.NWBIR74"],
...
all.equal(c(m3fb.imps[[1]]$indirect["ft.NWBIR74"],
...
# Total impact is linear combination
lc1 <- summary(glht(m3fb, linfct = c("ft.NWBIR74 + lag.ft.NWBIR74 = 0")))
lc2 <- summary(glht(m3fb, linfct = c("east + lag.east = 0")))
all.equal(c(m3fb.imps[[1]]$total["ft.NWBIR74"],
...
# 
# ### Test SDM
# m3 <- spatialreg::lagsarlm(ft.SID74 ~ ft.NWBIR74  + east, data=nc, listw=nc_lw, Durbin=TRUE)
# summary(m3)
# spatialreg::impacts(m3, listw=nc_lw)
# 
# 
# # Manually
# W <- listw2mat(nc_lw)
# M <- solve(diag(1, nrow = length(m3$residuals)) - m3$rho * W)
# b <- m3$coefficients[c("ft.NWBIR74", "east")]
# lb <- m3$coefficients[c("lag.ft.NWBIR74", "lag.east")]
# for(i in 1:2){
